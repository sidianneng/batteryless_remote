...
```
  ...
  for(j = 0;j < 5; j++)
  {
    ...
    {
      test_write_data[i] = i;
      Log_Printf("%02x ", test_write_data[i]);
    }
    Log_Printf("\n");
  ...
  for(j = 0;j < 5; j++)
  {
    ...
    Log_Printf("\n");
    ret = flash_write(IRDATA_START_ADDR, test_write_data, 256, 0);
    Log_Printf("same addr flash write ret:%d\n", ret);
    ret = flash_read(IRDATA_START_ADDR, test_read_data, 256);
    Log_Printf("same addr flash read ret:%d\n", ret);
    for(uint16_t i = 0;i < 256; ++i)
      Log_Printf("%02x ", test_read_data[i]);
    Log_Printf("\n");
    LL_mDelay(500000);
